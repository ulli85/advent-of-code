...
print(sum(list(map(lambda numbers: int(numbers[0] + numbers[-1]), map(lambda line: re.findall('\\d', line), list(map(lambda line: replace_2_digits(line), open("input/1.txt", "r").read().splitlines())))))))
