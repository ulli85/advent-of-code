...
def is_rectangle_inside_polygon_with_time(x1, y1, x2, y2):
    start_time = time.time_ns()
    rv = is_rectangle_inside_polygon(x1, y1, x2, y2)
    print(f'({x1}, {y1}) x ({x2}, {y2}) took {(time.time_ns() - start_time) / 1000_000} ms ---')
    return rv
...
def on_click(event):
    ...
    if is_point_inside_polygon(xp, yp, EDGES):
        print('inside')
...
def show_rays():
    xmin, xmax = min(map(lambda coord: coord[0], POINTS)), max(map(lambda coord: coord[0], POINTS))
    ymin, ymax = min(map(lambda coord: coord[1], POINTS)), max(map(lambda coord: coord[1], POINTS))
    for yp in range(ymin - 4, ymax + 4 + 1):
        for xp in range(xmin - 4, xmax + 4 + 1):
            color = 'go' if is_point_inside_polygon(xp, yp, EDGES) else 'ro'
            plt.plot(xp, yp, color, markersize=5)
...
#show_rays() # for smaller polygons only
plt.show()
...
while not rectangle_found:
    ...
    for (x1, y1), (x2, y2) in points_list:
        if is_rectangle_inside_polygon_with_time(x1, y1, x2, y2):
